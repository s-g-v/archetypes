...
public class ReportAppender extends AppenderBase<ILoggingEvent> {
	...
	public final void start() {
		...
		try {
			...
			Reporter.log("<script type=${symbol_escape}"text/javascript${symbol_escape}" src=${symbol_escape}"reportng.js${symbol_escape}"></script>");
		} catch (IOException e) {
			e.printStackTrace();
		}
		super.start();
	...
	private static String decorateLink(final String text) {
		...
		decoratedLink.append("<a href=${symbol_escape}"");
		decoratedLink.append(link);
		decoratedLink.append("${symbol_escape}">");
		decoratedLink.append(link);
	...
	private static String decorateWarn(final String text) {
		...
		decoratedWarn.append("<div style=${symbol_escape}"" + ReportColor.WARNING.getColor() + "${symbol_escape}">");
